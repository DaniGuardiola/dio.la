...
The word ["polymorphism"](<https://en.wikipedia.org/wiki/Polymorphism_(computer_science)>) sounds very complicated, but is fairly simple to understand in the context of components.
...
What if we want to add a tooltip to an action in the card?
...
> [The `title` HTML attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/title) behaves similar to a tooltip, but [it has significant accessibility issues](https://inclusive-components.design/tooltips-toggletips/).
...
<Ariakit.TooltipProvider>
  <Ariakit.TooltipAnchor render={<button>ACTION</button>} />
...
If we want to add a tooltip to it, this is more or less what we need to happen, in simplified pseudocode form:
...
The rendered DOM will have the following structure:

```html
<button>ACTION</button>
<div>My tooltip's content</div>
```

With proper styles, it will look like this:

![invert || A card that contains an action that's being hovered, making its tooltip visible](./tooltip.png)

For this to work with `<Ariakit.TooltipAnchor>`, we can't just naively pass the `<button>` as a child. That would cause it to become wrapped in a `<div>` (the actual tooltip anchor), which could break our layout and complicate things in many ways.

We don't need a wrapper, we only need the button to handle certain event handlers like `onPointerEnter` and `onPointerLeave`. To make `<button>` the anchor, what we need is to render `<TooltipAnchor>` "as" our `<button>`.
