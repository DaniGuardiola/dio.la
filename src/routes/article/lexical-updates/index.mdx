...
- The pending state becomes mutable. The update callback applies changes to it.
- Once the update callback is done, the pending state is "locked" (made immutable again).
...
Modules can expose values in their scope to the outside world through `export` statements. In fact, that's the point of a module! Modules usually look like this:

```ts twoslash include module-one
// @module: esnext
// @filename: module-one.ts
const somePrivateConstant = "my constant";
let somePrivateVariable = "my variable";

function somePrivateFunction() {
  // ...
}

export const somePublicConstant = "my public constant";
export let somePublicVariable = "my public variable";
...
Values that are not exported are private, because they cannot be imported from other modules.
