...
Okay, okay, don't go crazy just yet. There are really good answers to all of these questions. Spoiler alert: they're all related!
...
- The _pending_ state is set as the _current_ state, replacing the previous state.
- Reconciliation happens and the resulting changes are applied to the DOM. More on this later.
...
> Note that since the `LexicalEditor.update` method is already being called on an editor instance, we already know that that's the target editor (`this`). However, the actual function that contains the update logic is separate from this method (the method calls this function internally though), so we need to receive it as an argument.
